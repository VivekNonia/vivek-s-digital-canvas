    ...
    <section id="experience" className="section-container" ref={ref}>
      ...
      <div className="max-w-5xl mx-auto">
        {experiences.map((exp, index) => (
          <div key={index} className={`relative scroll-reveal ${isVisible ? `revealed delay-${(index + 2) * 200}` : ''}`}>
            {/* Timeline connector */}
            {index < experiences.length - 1 && (
              <div className="absolute left-8 top-20 bottom-0 w-px bg-gradient-to-b from-primary/50 to-border hidden md:block" />
            )}
            
            <div className="glass-card p-10 card-glow transition-all duration-300 hover:border-primary/40 relative overflow-hidden group hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/10 glow-effect">
              {/* Background glow */}
